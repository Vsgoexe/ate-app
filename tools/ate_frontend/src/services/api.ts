...
import { allowOfflineFallback, offlineFallback } from "@/lib/offlineApiFallback";
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404 && allowOfflineFallback()) {
          return fallbackOrThrow<T>(path, init?.method || "GET", new Error(`API ${path} not found`));
        }
      ...
      if (attempt < retries && !unauthorized) {
        await sleep(250 * 2 ** attempt);
        continue;
      }
      if (unauthorized) throw lastErr;
      return fallbackOrThrow<T>(path, init?.method || "GET", lastErr);
    }
  }
  throw lastErr;
}
