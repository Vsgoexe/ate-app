...
export function DashboardShell() {
  const { summary, isLoading, isError, error, refetch } = useDashboardData();
  ...
  if (isError && !summary) {
    return (
      <div className="mx-auto max-w-[1400px] px-7 pb-[90px] pt-[30px]">
        <header className="vl-header mb-[28px]">
          <VerilumenBrand size="header" />
        </header>
        <ErrorState
          message={
            error instanceof Error
              ? `Unable to load dashboard summary from the API. ${error.message}`
              : "Unable to load dashboard summary from the API."
          }
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-7 pb-[90px] pt-[30px]">
      ...
      <div className="vl-section-enter vl-section-enter-delay-3">
        <OptimizationKpiGrid
          dtlSlot={<DynamicTestLimits data={summary?.test_limits ?? null} />}
        >
