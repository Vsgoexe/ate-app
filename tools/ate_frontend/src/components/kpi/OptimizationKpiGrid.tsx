...
import { agentHref, KPI_TO_AGENT } from "@/lib/agents";
import {
  SHMOO_CAPABILITIES,
...
export function OptimizationKpiGrid({
  ...
  function openAgent(kpiId: string) {
    const agentId = KPI_TO_AGENT[kpiId];
    if (agentId) router.push(agentHref(agentId));
  }
  ...
  return (
    <div className="mb-[26px] grid grid-cols-1 items-stretch gap-3.5 sm:grid-cols-2">
      {ordered.map((kpi) => (
        <Fragment key={kpi.id}>
          {kpi.id === "false_failure_reduction" ? (
            dtlSlot
          ) : (
            <OptimizationKpiCard
              kpi={{ ...kpi, name: displayName(kpi.id, kpi.name) }}
              onOpen={KPI_TO_AGENT[kpi.id] ? openAgent : undefined}
