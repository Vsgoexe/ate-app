...
export function OptimizationKpiCard({
  ...
  return (
    <div
      ...
      onClick={clickable ? () => onOpen?.(kpi.id) : undefined}
      onKeyDown={
        clickable
          ? (e) => {
      ...
      className={`vl-card relative flex h-full w-full flex-col overflow-hidden text-left ${
        ...
        clickable
          ? "cursor-pointer transition-[border-color,box-shadow] duration-200 hover:border-[rgba(107,193,242,0.45)] hover:shadow-[0_0_0_1px_rgba(107,193,242,0.18)]"
          : ""
      }`}
    >
