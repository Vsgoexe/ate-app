...
from dtl_agent.api.demo_cache import demo_cache_ready, restore_demo_session_from_cache
...
def _demo_datasets_root() -> Path:
    ...
    for candidate in (
        here.parents[6] / "demo_datasets",
        here.parents[7] / "demo_datasets",
        here.parents[5] / "demo_datasets",
    ):
        if candidate.is_dir():
...
@router.post(
    ...
    summary="Load bundled three-month demo ZIPs and start analysis",
)
async def post_analysis_demo_load(
    _: ReadyDep,
    project_root: ProjectRootDep,
) -> dict:
    """Presentation mode: auto-load Jan/Feb/Mar demo ZIPs from demo_datasets/dtl/."""
    try:
        demo_datasets = _demo_datasets_root()
        clear_analysis_cache()
        clear_cost_savings_caches()
        clear_die_level_process_caches()

        if demo_cache_ready(demo_datasets):
            cached = restore_demo_session_from_cache(demo_datasets)
            if cached is not None:
                return cached

        demo_root = demo_datasets / "dtl"
        files = {
            "january": (
                "dtl_input_2026_01.zip",
                (demo_root / "dtl_input_2026_01.zip").read_bytes(),
            ),
            "february": (
                "dtl_input_2026_02.zip",
                (demo_root / "dtl_input_2026_02.zip").read_bytes(),
            ),
            "march": (
                "dtl_input_2026_03.zip",
                (demo_root / "dtl_input_2026_03.zip").read_bytes(),
            ),
        }
        for slot, (name, payload) in files.items():
            if not payload:
                raise _validation(f"Demo file missing for {slot}: {name}")
        res = start_upload_analysis_job(files=files, source_root=project_root)
        res["demo"] = True
        return res
        ...
        raise ServiceError(
            code="DEMO_LOAD_ERROR",
            message=f"Demo analysis load failed: {type(exc).__name__}: {exc}",
