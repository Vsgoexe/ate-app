...
export function ThreeMonthDashboardPage() {
  ...
  useEffect(() => {
    if (!isDemoMode() || demoLoadStarted.current) return;
    demoLoadStarted.current = true;
    const ctrl = new AbortController();
    setError(null);
    setAnalyzing(true);
    setJobStage("Loading demo datasets…");
    setProgressPct(5);
    postDemoAnalysisUpload(ctrl.signal)
      .then((result) => {
      ...
      .catch((err: unknown) => {
        ...
        if (err instanceof ApiError) {
          ...
          setError("Failed to load demo datasets.");
        }
        setAnalyzing(false);
        setJobStage(null);
        setProgressPct(null);
  ...
  return (
    <div className="space-y-5" data-testid="three-month-dashboard">
      ...
      {!sessionId && !error && !analyzing && !isDemoMode() ? (
      ...
      {error ? <ErrorState message={error} code={errorCode} /> : null}

      {sessionId && (analyzing || loading) ? (
        <div data-testid="analyzing-uploaded" className="space-y-2">
          <LoadingState message={jobStage || "Analyzing uploaded Jan/Feb/Mar data..."} />
        </div>
      ) : null}
