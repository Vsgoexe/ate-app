  ...
  if (view === 'benefit_rate' || view === 'unnecessary_retests') {
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '16px' }}>
          {isBenefit ? (
            `${kpis?.benefit_events_count || 0} / ${counts.retest} = ${Number(kpis?.benefit_rate_pct ?? 0).toFixed(2)}% — Definition: share of AI RETEST recommendations whose actual outcome was RETEST_BENEFICIAL.`
          ) : (
            `${kpis?.fp || 0} / ${kpis?.total_events || 0} = ${Number(kpis?.unnecessary_retests_pct ?? 0).toFixed(2)}% — Definition: events where AI recommended RETEST but the actual outcome was PERSISTENT_FAILURE.`
          )}
        </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            {outcomesLoaded && validationData ? (
              <KpiCard
                ...
                value={`${Number(validationData.kpis?.benefit_rate_pct ?? 0).toFixed(2)}%`}
            ...
            {outcomesLoaded && validationData ? (
              <KpiCard
                ...
                value={`${Number(validationData.kpis?.unnecessary_retests_pct ?? 0).toFixed(2)}%`}
