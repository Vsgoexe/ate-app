...
@app.after_request
def allow_dashboard_iframe(response):
    response.headers["Content-Security-Policy"] = IFRAME_ANCESTORS
...
def _demo_datasets_root() -> Path:
    ...
    if env:
        ...
        if root.is_dir():
            return root.resolve()
    for candidate in (
        BASE_DIR.parent.parent / 'demo_datasets',
        BASE_DIR.parent.parent.parent / 'demo_datasets',
        BASE_DIR.parent / 'demo_datasets',
    ):
        if candidate.is_dir():
...
def _process_dataset_file(save_path: Path) -> dict:
...
def upload():
    ...
    try:
        ...
        return jsonify(_process_dataset_file(save_path))
...
def demo_load():
    """Load bundled demo CSV for presentations (same pipeline as /api/upload)."""
    try:
        demo_csv = _demo_datasets_root() / 'shmoo' / 'Normal_Shmoo_Dataset_500.csv'
        if not demo_csv.is_file():
            return jsonify({'error': f'Demo dataset not found: {demo_csv}'}), 404
        session_id = str(uuid.uuid4())
        save_path = UPLOAD_DIR / f"{session_id}_demo.csv"
        save_path.write_bytes(demo_csv.read_bytes())
        payload = _process_dataset_file(save_path)
        payload['demo'] = True
        payload['source_file'] = demo_csv.name
