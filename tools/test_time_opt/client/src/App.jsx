      ...
      try {
        ...
        for (const item of manifest.files || []) {
          const fileRes = await fetch(
            `${API}/api/demo/logs/file?rel=${encodeURIComponent(item.rel)}`
          );
          if (!fileRes.ok) continue;
          const text = await fileRes.text();
          const blob = new Blob([text], { type: "text/plain" });
          entries.push({
            file: new File([blob], item.name, { type: "text/plain" }),
            rel: item.rel,
          });
        }
        setLogFiles(entries);
    ...
    <div className="app">
      ...
      {processMode === "cost" && (
        <CostSavingsPanel
          ...
          onAteBurdenRateChange={(v) => {
            if (v === "") {
              setAteBurdenRate("");
              return;
            }
            const n = Number(v);
            if (Number.isFinite(n)) setAteBurdenRate(n);
          }}
          onDiesPerYearChange={(v) => {
            if (v === "") {
              setDiesPerYear("");
              return;
            }
            const n = Number(v);
            if (Number.isFinite(n)) setDiesPerYear(n);
          }}
        />
      )}
    </div>
